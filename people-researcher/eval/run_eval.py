...
def transform_dataset_inputs(inputs: dict) -> dict:
    ...
    return {
        ...
        # Add configurable options for the agent
        "configurable": {
            "search_provider": os.environ.get("SEARCH_PROVIDER", "combined"),
            "llm_provider": os.environ.get("LLM_PROVIDER", "gemini"),
            "gemini_model": os.environ.get("GEMINI_MODEL", "gemini-2.0-flash"),
            "enable_grounding_search": os.environ.get("ENABLE_GROUNDING_SEARCH", "true").lower() == "true"
        }
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--search-provider",
        type=str,
        choices=["gemini", "firecrawl", "serpapi", "combined"],
        default="combined",
        help="Search provider to use for evaluation",
    )
    parser.add_argument(
        "--llm-provider",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Gemini model to use (if Gemini is the LLM provider)",
    )
    parser.add_argument(
        "--enable-grounding-search",
        action="store_true",
        default=True,
        help="Enable Gemini's grounding search capability",
    )
    ...
    os.environ["ENABLE_GROUNDING_SEARCH"] = str(args.enable_grounding_search)
