...
def check_gemini_status():
    ...
    try:
        ...
        # Try to list models - this will fail if the API is down
        models = genai.list_models()
        return len(models) > 0
    except Exception as e:
        logging.error(f"Failed to check Gemini API status: {str(e)}")
        return False
...
def generate_pdf(query, data, summary, deep_research=False):
    ...
    # Metadata
    story.append(Paragraph(f"Gemini API Status: {'Operational' if check_gemini_status() else 'Down'}", styles['Normal']))
...
def generate_docx(query, data, summary, deep_research=False):
    ...
    doc.add_paragraph(f"Gemini API Status: {'Operational' if check_gemini_status() else 'Down'}")
...
st.write("Enter a query to research and get a detailed response using Gemini and FireCrawl.")

# Sidebar: Status and Info
st.sidebar.header("Gemini API Status")
...
st.sidebar.write("Dual-agent system using Gemini 2.0 Flash for research and content generation.")
...
if st.button("Run Research"):
    ...
    elif not check_gemini_status():
        st.error("Gemini API is currently down. Please try again later.")
    else:
        try:
